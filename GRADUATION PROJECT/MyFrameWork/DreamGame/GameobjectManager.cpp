...
void GameobjectManager::BuildObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	...
	m_pSqureObject->SetTexture(L"Image/stones.dds", RESOURCE_TEXTURE2D,3);
	...
	m_pSqure2Object->SetTexture(L"Image/stones.dds", RESOURCE_TEXTURE2D,3);
	...
	m_pWarriorObject = new GameObject(UNDEF_ENTITY);
	m_pWarriorObject->InsertComponent<RenderComponent>();
	m_pWarriorObject->InsertComponent<CLoadedModelInfoCompnent>();
	m_pWarriorObject->SetPosition(XMFLOAT3(50, 0, 0));
	m_pWarriorObject->SetScale(10.f, 10.f, 10.f);
