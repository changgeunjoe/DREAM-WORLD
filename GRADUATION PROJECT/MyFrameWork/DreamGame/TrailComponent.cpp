...
void TrailComponent::RenderAstar(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, vector<int>& vecNodeQueue)
{
	...
	for (auto iter = vecNodeQueue.begin(); iter != vecNodeQueue.end(); iter++) {
		...
		xmf3Pos.push_back(Vector3::Add(g_bossMapData.GetTriangleMesh(*iter).GetVertex1(), XMFLOAT3(0, (float)i + 0.15f, 0)));
		xmf3Pos.push_back(Vector3::Add(g_bossMapData.GetTriangleMesh(*iter).GetVertex2(), XMFLOAT3(0, (float)i + 0.15f, 0)));
		xmf3Pos.push_back(Vector3::Add(g_bossMapData.GetTriangleMesh(*iter).GetVertex3(), XMFLOAT3(0, (float)i + 0.15f, 0)));
		i++;
	}
