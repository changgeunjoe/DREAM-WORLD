...
void SkinnedMeshComponent::PrepareSkinning(GameObject* pModelRootObject)
{
...
void SkinnedMeshComponent::LoadSkinInfoFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
...
void SkinnedMeshComponent::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void SkinnedMeshComponent::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	if (m_pd3dcbBindPoseBoneOffsets)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(13, d3dcbBoneOffsetsGpuVirtualAddress); //Skinned Bone Offsets
	}
	...
	if (m_pd3dcbSkinningBoneTransforms)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(14, d3dcbBoneTransformsGpuVirtualAddress); //Skinned Bone Transforms
...
void SkinnedMeshComponent::ReleaseShaderVariables()
{

}

void SkinnedMeshComponent::ReleaseUploadBuffers()
{
	StandardMeshComponent::ReleaseUploadBuffers();
...
void SkinnedMeshComponent::OnPreRender(ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[7] = { m_d3dVertexBufferView, m_d3dTextureCoord0BufferView, m_d3dNormalBufferView, m_d3dTangentBufferView, m_d3dBiTangentBufferView, m_d3dBoneIndexBufferView, m_d3dBoneWeightBufferView };
